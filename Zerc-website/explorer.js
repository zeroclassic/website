...
function buildChart() {
    $(document).ready(function() {
        ...
        $("#networkHashrate").click(function() {
            networkHashrate();
        ...
        $("#blockDifficulty").click(function() {
            blockDifficulty();
            $(this).siblings().removeClass('active')
            $(this).addClass('active');
        });
        ...
        function blockDifficulty() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    for (var i = 0; i < data.length; i++) {
                        blockDifficultyPoints.push({
                            x: data.blocks[i].height,
                            y: data.blocks[i].difficulty
                        });
                    }
                    ...
                    var chart = new CanvasJS.Chart("charts", {
                        ...
                        data: [{
                            type: "spline",
                            color: "#47358f",
                            dataPoints: blockDifficultyPoints
                        }]
                    });
        ...
        function networkHashrate() {
            var networkHashratePoints = []
            $.ajax({
                type: 'GET',
                url: 'https://pool.zeroclassic.org/api/pool_stats',
                dataType: 'json',
                success: function(data) {
                    for (var i = 0; i < data.length; i++) {
                        networkHashratePoints.push({
                            x: ((data[i].time*1000)),
                            y: parseInt(data[i].pools.zeroclassic.networkSols)
                        });
                    }
                    ...
                    var chart = new CanvasJS.Chart("charts", {
                        ...
                        axisY:{
                            title:"Hashrate (Sol/s)",
                        },
                        data: [{
                            type: "spline",
                            color: "#47358f",
                            xValueType: "dateTime",
                            dataPoints: networkHashratePoints
                        }]
                    });
